...
out_root = r'E:\2exp_fMRI\Exp\Data\Data\fmri\BIDS\derivatives\MVPA\RSA_blocks_6HMP_demeaned_norest_corr'
...
for conditions in conditions_list:
    ...
    ylim_top = math.ceil(np.max(np.hstack((np.array(same_avg) + np.array(
        same_yerr), np.array(diff_avg) + np.array(diff_yerr)))) * 10 ** 1) / 10 ** 1
    ylim_bottom = math.floor(np.min(np.hstack((np.array(same_avg) - np.array(
        same_yerr), np.array(diff_avg) - np.array(diff_yerr)))) * 10 ** 1) / 10 ** 1
